...
async function getAllDeals() {
  ...
  const { data, error } = await supabase
    .from('deals')
    .select('id, plan_name, description, price, retail_price, is_featured, is_member_exclusive, is_negoshi_pick, affiliate_url, speed_down, has_lock_in, speed_down, has_lock_in, providers ( name, logo_color, logo_text ), categories ( name, slug )')
    .eq('is_active', true)
        ...
        <div style={{ background: '#F7F4EE', padding: '4rem 5% 3rem' }}>
          <div style={{ maxWidth: 1160, margin: '0 auto' }}>
            ...
            <h1 style={{ fontFamily: 'var(--font-display), Fraunces, serif', fontSize: 'clamp(2.2rem, 4vw, 3.25rem)', fontWeight: 700, letterSpacing: '-0.03em', lineHeight: 1.1, color: '#1A1714', marginBottom: '0.75rem' }}>
              The best deals in Australia, right now.
            </h1>
            <p style={{ fontSize: '1rem', color: '#7A736C', lineHeight: 1.7, maxWidth: 500 }}>
              Every deal checked and updated daily. No paid placements.
            </p>
          </div>
        </div>
